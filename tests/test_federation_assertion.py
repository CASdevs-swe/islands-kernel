...
from identity.federation.assertion import verify_island_assertion, IslandAssertionError, verify_island_assertion_symmetric
...
# ── Symmetric (HS256) verifier tests ────────────────────────────────────────

SHARED = "s" * 32


def _assert_hs(*, sub="42", nonce="n1", aud=AUD, iss=ISS, exp=2000, email="a@b.se", workspace="ws_7", secret=SHARED):
    claims = {"iss": iss, "sub": sub, "aud": aud, "nonce": nonce, "exp": exp,
              "email": email, "workspace": workspace}
    return pyjwt.encode(claims, secret, algorithm="HS256")


def test_symmetric_valid_assertion_returns_identity():
    tok = _assert_hs()
    out = verify_island_assertion_symmetric(tok, secret=SHARED, expected_iss=ISS,
        expected_aud=AUD, expected_nonce="n1", now=1000)
    assert out == {"island_user_id": "42", "email": "a@b.se", "workspace": "ws_7"}


def test_symmetric_wrong_secret_is_rejected():
    tok = _assert_hs(secret="x" * 32)
    with pytest.raises(IslandAssertionError):
        verify_island_assertion_symmetric(tok, secret=SHARED, expected_iss=ISS,
            expected_aud=AUD, expected_nonce="n1", now=1000)


def test_symmetric_nonce_mismatch_is_rejected():
    tok = _assert_hs(nonce="other")
    with pytest.raises(IslandAssertionError):
        verify_island_assertion_symmetric(tok, secret=SHARED, expected_iss=ISS,
            expected_aud=AUD, expected_nonce="n1", now=1000)


def test_symmetric_expired_is_rejected():
    tok = _assert_hs(exp=900)
    with pytest.raises(IslandAssertionError):
        verify_island_assertion_symmetric(tok, secret=SHARED, expected_iss=ISS,
            expected_aud=AUD, expected_nonce="n1", now=1000)


def test_symmetric_alg_confusion_eddsa_token_rejected():
    # an EdDSA-signed token must not verify on the HS256 path
    km = KeyManager.generate("island-1")
    tok = _assert(km)  # EdDSA helper from this file
    with pytest.raises(IslandAssertionError):
        verify_island_assertion_symmetric(tok, secret=SHARED, expected_iss=ISS,
            expected_aud=AUD, expected_nonce="n1", now=1000)
